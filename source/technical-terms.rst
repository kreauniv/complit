...
The field of Computer Science uses a few technical terms in place of the common
...
**Procedures and Functions**
    ...
    for the values that such words refer to. Note that it gets tedious quickly
    if we keep maintaining the distinction between a word and the value it
...
**Higher order procedures**
    ...
    list of characters parsed by, say, ``(one-or-more digit)``. Such a
    procedure is another kind of **higher order procedure** and may also be
    called a **higher order function**, or **HOF** for short, depending on context.
    There are many such HOFs provided for you by Racket which make expressing
    common ideas simple -- such as ``map``, ``filter``, ``andmap``, ``ormap``,
    and ``for-each``.
