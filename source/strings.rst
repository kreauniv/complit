...
to refer to it. In Racket, and in many programming languages, this role is
played by an **identifier**. If "Magnificent flying machines" happened to be
the title of an article, we might bind the ``title`` identifier to the string
value by typing this into the "definitions window" and clicking on "Run" --
...
4. ``(string-ref <str> <cursor-pos>)`` gets you the character to the right of
   the given cursor position.

5. ``(string-append <str1> <str2> ...)`` constructs a new string that is a
   concatenation of the argument strings. The arguments must all refer to
   strings or else the construct is considered invalid.

6. ``(substring <str> <start-cursor-pos> <end-cursor-pos>)`` gives a string
   that has the characters between the two given cursor positions. If you omit
   the end position, it is taken to be the last valid cursor position --
   right-most in left-to-right scripts, and left-most in right-to-left scripts.
